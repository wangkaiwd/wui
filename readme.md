...
* 有些文件为什么要加`_`
* 在出现模态框后并进行上下滑动，划出后会出现白色区域。这个可能和北京色透明有关系。
...
### 目前项目中存在的问题
* 目录结构不够清晰
* `css`结构不清晰，导致样式覆盖不太容易，并且有很多全局样式会相互影响
* 发布到`npm`后下载无法使用
* 代码使用展示的样式以及源代码中含有其它多余的内容
* `exapmle`文件和`raw-loader`的结合还需要进行思考

### 困惑
* 打包后本地`server`预览正常，但是部署到`github page`预览样式会发生一些问题
解决方法：  
通过`command+0`重置浏览器的正常大小，原因并不太懂。 这个问题告诉我们，当发现页面在相同环境的显示效果不一样的时候，可以看看是不是有缩放的情况。
